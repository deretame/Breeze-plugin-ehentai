...
function handleHealth(req: IncomingMessage, res: ServerResponse): void {
  ...
  writeJson(res, state.ok ? 200 : 503, {
    ok: state.ok,
    builtAt: state.builtAt,
    sha256: state.sha256,
    size: state.size,
    error: state.error,
    bundleUrl: `${origin}${bundleRoutePath}`,
    endpoints: {
      search: `${origin}${API_PATHS.search}`,
      detail: `${origin}${API_PATHS.detail}`,
      read: `${origin}${API_PATHS.read}`,
      log: `${origin}${API_PATHS.log}`,
    },
  });
}
...
function handleDefault(req: IncomingMessage, res: ServerResponse): void {
  ...
  res.end(
    [
      ...
      `health: ${origin}${API_PATHS.health}`,
      `search: ${origin}${API_PATHS.search}`,
      `detail: ${origin}${API_PATHS.detail}`,
      `read:   ${origin}${API_PATHS.read}`,
      `log:    ${origin}${API_PATHS.log}`,
...
async function runApiHandler<T>(
  req: IncomingMessage,
  res: ServerResponse,
  invoke: (payload: Record<string, unknown>) => Promise<T>,
): Promise<void> {
  if (req.method !== "POST") {
    writeJson(res, 405, { ok: false, error: "method not allowed" });
    return;
...
async function handleSearch(req: IncomingMessage, res: ServerResponse): Promise<void> {
  await runApiHandler(req, res, async (payload) => {
    return await searchComic(payload);
  });
}

async function handleDetail(req: IncomingMessage, res: ServerResponse): Promise<void> {
  await runApiHandler(req, res, async (payload) => {
    return await getComicDetail(payload);
  });
}
...
async function route(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (pathname === API_PATHS.health) {
    handleHealth(req, res);
    return;
  }

  if (pathname === API_PATHS.search) {
